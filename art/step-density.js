import { el, text, rect, circle, line, group, swatch, ready, stepHeader } from "./svg.js";
...
async function main() {
  ...
  svg.append(stepHeader("02 / FIND PEAKS", "Find the color families", [
    "Samples are binned in OKLAB. The",
    "densest bins that sit at least 0.10",
    "apart become family peaks, so a",
    "small color far from the rest is",
    "always its own family.",
  ]));

  const ox = 520, oy = 70, os = 340, range = 0.2;
  ...
  svg.append(text(ox + os - 6, Y(0) - 6, "+a", { class: "label-muted", "text-anchor": "end", style: "font-size:9px;text-transform:none" }));
  svg.append(text(X(0) + 6, oy + 14, "+b", { class: "label-muted", style: "font-size:9px;text-transform:none" }));
  ...
  svg.append(text(ox, oy + os + 22, "OKLAB a/b PLANE · BIN 0.02 · SEPARATION 0.10", { class: "label" }));
  ...
  if (overlap) {
    svg.append(text(ox, oy + os + 40, "(distance is weighted, so peaks may overlap here)", { class: "label-muted", style: "font-size:9px" }));
  }
  ...
  a.families.forEach((f, i) => {
    const yy = ly + 24 + i * 30;
    svg.append(swatch(lx, yy - 14, 36, 20, f.rgb));
    svg.append(text(lx + 46, yy, f.hex, { class: "value" }));
    svg.append(text(lx + 130, yy, `${(f.share * 100).toFixed(1)}%`, { class: "value", fill: "#9db2c0" }));
    svg.append(text(lx + 190, yy, f.lch.chroma < 0.03 ? "gray" : `${Math.round(f.lch.hue)}°`, { class: "value", fill: "#7e8e94" }));
  });
